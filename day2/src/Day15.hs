...
step :: HashSet (Int, Int) -> Path -> ((HashSet (Int, Int)), MinQueue Path)
step _ (Path cave []) = ((HashSet.singleton (0, 0)), (MinQueue.singleton $ Path cave [(0, 0)]))
step visited (Path cave@(Cave width height riskLevels) path@(coord : tail))
  ...
  | otherwise =
    let validCoord coord@(x, y) = x >= 0 && x <= width && y >= 0 && y <= height && (not . HashSet.member coord $ visited)
        adj = filter validCoord $ adjacent coord
        expanded = (: coord : tail) <$> adj
        queue = MinQueue.fromList (Path cave <$> expanded)
        nextVisited = visited `union` (HashSet.fromList adj)
...
--    in -- noRedundant = filter (all (not . (`HashSet.member` endPoints)) . tail) deduped
--       -- noRedundant =
--       --   filter
--       --     ( \p ->
...
findSafePath cave@(Cave width height _) =
  let go :: HashSet (Int, Int) -> MinQueue Path -> IO Path
      go visited paths =
        case MinQueue.minView paths of
          Nothing -> go (HashSet.singleton (0, 0)) (MinQueue.singleton $ Path cave [(0, 0)])
          Just (p, ps)
