...
unit_invertAmod14Eq7 =
  let inverted = invertInstruction (Mod X (Reg Y)) 7
      f = _r2ToR1 inverted
      as = take 5 $ f 14
   in as `shouldBe` [7, 21, 35, 49, 63]

unit_invertAMod3Eq3 =
  let inverted = invertInstruction (Mod X (Reg Y)) 3
...
unit_invert6ModBEq5 =
  let inverted = invertInstruction (Mod X (Reg Y)) 5
      as = _r1ToR2 inverted 6
   in as `shouldBe` []

unit_invert5MulBEq12 =
  let inverted = invertInstruction (Mul X (Reg Y)) 12
...
unit_invert5DivBEq2 =
  let inverted = invertInstruction (Div X (Reg Y)) 2
      as = _r1ToR2 inverted 5
   in as `shouldBe` [2]

unit_invert6DivBEq7 =
  let inverted = invertInstruction (Div X (Reg Y)) 7
      as = _r1ToR2 inverted 6
   in as `shouldBe` []

unit_invertBDiv3Eq6 =
  let inverted = invertInstruction (Div X (Reg Y)) 6
      as = _r2ToR1 inverted 3
   in as `shouldBe` [18, 19, 20]

unit_invertBDiv8Eq0 =
  let inverted = invertInstruction (Div X (Reg Y)) 0
